...
// Update phone START
router.put('/modifPhone/:id',(req, res) =>{
  Phone.findById({ _id:req.params.id }, (error, phone) => {
        ...
        console.log(phone);
          ...
          if (error) {
            console.log(error);
             ...
             console.log(phone);
            res.send("good. phone is updated with success");
          }
        });
      }
    });
...
router.delete('/deletePhone/:id',(req, res) =>{
 Phone.deleteOne({ _id: req.params.id }, (err, phone) => {
   if (err) {
      console.log(err);
      res.send(err);
    } else {
